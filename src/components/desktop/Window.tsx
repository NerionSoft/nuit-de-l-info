  ...
  return (
    <AnimatePresence>
      <Rnd
        ...
        onMouseDown={(e) => {
          // Only focus if clicking on the window frame, not the content
      ...
      >
        <motion.div
          ...
          className={cn(
            'flex flex-col h-full bg-[#0F172A] rounded-lg overflow-hidden shadow-2xl',
            isActive ? 'ring-2 ring-[#3B82F6]/50' : 'ring-1 ring-white/10'
          )}
        >
          {/* Title Bar */}
          <div
            className={cn(
              ...
              isActive ? 'bg-[#3C0028]' : 'bg-[#0F172A]'
            )}
          >
            {/* Window Controls (Left side - Linux style) */}
            <div className="flex items-center gap-2">
              <button
                onClick={() => closeWindow(win.id)}
                className="group w-3.5 h-3.5 rounded-full bg-[#3B82F6] hover:bg-[#FF6B35] flex items-center justify-center transition-colors"
              >
